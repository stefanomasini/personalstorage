...
export async function auth(): Promise<void> {
  ...
  if (!result.refresh_token) {
    console.error("No refresh token in response. Make sure token_access_type is 'offline'.");
    process.exit(1);
  }

  console.log("\nRefresh token obtained. Store it in envchain:\n");
  console.log(`  envchain personalstorage set DROPBOX_REFRESH_TOKEN`);
